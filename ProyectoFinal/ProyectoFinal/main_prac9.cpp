...
float roundos = 0.0f;
float atras = 0.02f;
float enfrente = 20.0f;
float arriba = 20.0f;
float abajo = 20.0f;
float enfrente2 = 20.0f;
float movAuto_z = 0.0f;
float movAuto_x = 15.0f;
...
void animate(void)
{
	if (animacion)
	{
		if (atras <= 7.0f){
			...
			if (arriba > 5.5f){
				enfrente = 0.0f;
				printf("Posicion en y %f \n", movAuto_y + 1.75f); //Quitandole el estado inicial, para saber realmente cuanto se desplaza
			}				
		}else if (enfrente <= 11.5f) {
			movAuto_z += 0.02f;
			enfrente += 0.02f;
			if (enfrente > 11.5f) {
				abajo = 0.02f;
...
void display(Shader shader, Shader skyboxShader, GLuint skybox, Model modelo, Model llantas, Model piso)
{
	...
	tmp = model = glm::translate(model, glm::vec3(movAuto_x, movAuto_y, movAuto_z));
...
void my_input(GLFWwindow *window, int key, int scancode, int action, int mode)
{
	...
	if (glfwGetKey(window, GLFW_KEY_SPACE) == GLFW_PRESS) {
		animacion = true;
		atras = 0.02f;
		arriba = enfrente = abajo = enfrente2 = 20.0f;
		movAuto_z -= movAuto_z;
	}
